...
M. Amaral等人的工作综合考虑了多项指标，以使得集群达到一个最优状态。在这样的集群中，所有的任务都被视作拥有同样的优先级。因此，可能存在集群中常见的长尾问题。图~\ref{hived_pending}就描述了这样一个场景。租户A和租户B都需要使用GPU，然而由于调度问题，虽然集群中还剩余4个GPU，却无法分配给B。

\begin{figure}[h]
    \centerline{\includegraphics[width=\textwidth]{figures/hived-pendding.png}}
    \caption{GPU集群中一个任务被阻塞。}
    \label{hived_pending}
\end{figure}
